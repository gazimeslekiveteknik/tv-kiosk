...
(function () {
    ...
    function applyAutoScaling() {
        ...
        setFixedCanvas(document.getElementById('layout-wrapper')); 
    ...
    function updateLessonTimer() {
        ...
        if(!els.statusSabahci || !els.statusOglenci) return;
        ...
        if (events.length > 0) {
            ...
            if(els.lessonProgress) els.lessonProgress.style.width = `${Math.max(0, Math.min(100, prog))}%`;
        } else {
            els.lessonTimer.textContent = "--:--";
            if(els.lessonProgress) els.lessonProgress.style.width = "0%";
        }
    }
